...
- 資料時效：整個資料庫每日 T-1 更新，所有日期欄位（SNAP_DATE、SNAP_YYYYMM、txn_date、open_date 等）的最新可用資料為昨日（SYSDATE-1）。當使用者說「今天」、「當日」、「本日」，一律解讀為昨日（SYSDATE-1）；禁止以今日日期（SYSDATE 或 DATE 'YYYY-MM-DD' 等於今日）作為篩選上限，否則查詢結果為空。取最新 SNAP_YYYYMM 時，必須使用 TO_CHAR(TRUNC(SYSDATE-1,'MM'),'YYYYMM')（T-1 所在月份）；禁止用 ADD_MONTHS(TRUNC(SYSDATE,'MM'),-1)，那會得到上個月的資料而非最新月份。
...
def _step_a(
    ...
    cases_text: str = "",
    user_profile: str = "",
    resolved_product_routes: str = "",
) -> tuple[str, str, int, int]:
    """Step A：LLM 從候選池生成 SQL + 思路（附參考案例）。回傳 (sql, reasoning, in_tok, out_tok)。"""
    optional_blocks = []
    if resolved_product_routes.strip():
        optional_blocks.append(resolved_product_routes)
...
def generate(
    ...
    step_a_sql, step_a_reasoning, a_in, a_out = _step_a(
        ...
        resolved_product_routes=resolved_product_routes,
    )
