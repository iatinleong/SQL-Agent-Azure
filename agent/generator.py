...
- 任何兩張表以個人關聯時，JOIN 條件必須涵蓋目標表格的所有複合 PK 欄位；若目標表以 party_id + brand_code 為複合 PK，則必須同時包含兩者；若目標表以 party_id 為單一 PK，則只需 party_id。
- 個人層級以 party_id 識別；帳戶層級以 acct_nbr + prod_type_code 識別（複合鍵）。層級由高到低：分公司 → 個人(party_id) → 帳戶(acct_nbr + prod_type_code)。
- party_id 套用 Oracle Data Redaction：任何 SELECT 清單（含 CTE 內層）禁止出現 party_id。
- party_id_mask 必須且只能從 DM_S_VIEW.M_AC_ACCOUNT 取得，依以下順序判斷：
  (1) 若目前查詢的 FROM/JOIN 已包含 DM_S_VIEW.M_AC_ACCOUNT alias，直接 SELECT 該 alias.party_id_mask；
  (2) 只有當目前查詢尚未包含 M_AC_ACCOUNT、且主表有 party_id 需要轉成 party_id_mask 時，才 JOIN DM_S_VIEW.M_AC_ACCOUNT ON <主表>.party_id = M_AC_ACCOUNT.party_id；
  (3) 禁止為了取得 party_id_mask 對 M_AC_ACCOUNT 做 self-join；
  (4) 禁止直接 SELECT 其他表格的 party_id_mask。
- JOIN / ON / WHERE 條件可使用任何表格的 party_id。
