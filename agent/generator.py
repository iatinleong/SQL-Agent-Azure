...
- 日期欄位比較規則（SYSDATE 含時間成分，裸用會導致等值比較不符）：
  ① DATE 型欄位（snap_date、open_date 等）等值比較：用 TRUNC(SYSDATE)-N，例如 snap_date = TRUNC(SYSDATE)-1。
  ② YYYYMM 型欄位（snap_yyyymm）等值比較：用 TO_CHAR(TRUNC(SYSDATE-N,'MM'),'YYYYMM')，例如 snap_yyyymm = TO_CHAR(TRUNC(SYSDATE-1,'MM'),'YYYYMM')。
  ③ 交易日期範圍（txn_date）：用 >= 起日 AND < 迄日+1（或 BETWEEN TO_DATE(...) AND TO_DATE(...)），迄日用 TO_DATE 確保不含時間。
- 資料時效：整個資料庫每日 T-1 更新，所有日期欄位（SNAP_DATE、SNAP_YYYYMM、txn_date、open_date 等）的最新可用資料為昨日（SYSDATE-1）。當使用者說「今天」、「當日」、「本日」，一律解讀為昨日（SYSDATE-1）；禁止以今日日期（SYSDATE 或 DATE 'YYYY-MM-DD' 等於今日）作為篩選上限，否則查詢結果為空。取最新 SNAP_YYYYMM 時，必須使用 TO_CHAR(TRUNC(SYSDATE-1,'MM'),'YYYYMM')（T-1 所在月份）；禁止用 ADD_MONTHS(TRUNC(SYSDATE,'MM'),-1)，那會得到上個月的資料而非最新月份。★ 取參考表（帳戶、名稱、標籤等）最新快照時，必須用子查詢 MAX(snap_yyyymm) 搭配上限 <= TO_CHAR(TRUNC(SYSDATE-1,'MM'),'YYYYMM')，禁止用 ADD_MONTHS(...,-N) 固定回推 N 個月——那會取到 N 個月前的舊資料而非最新可用快照。★ 禁止在 snap_yyyymm 條件中寫入尚未到達的未來月份（如今年 6 月就寫 '202612'），未來月份無資料會導致 CTE 回傳零筆；取某年度最新快照請用 MAX(snap_yyyymm) WHERE snap_yyyymm BETWEEN '起始月' AND '結束月'。★ 帳戶快照欄位對應：M_AC_ACCOUNT、M_AC_ACCOUNT_INFO、M_AC_ACCOUNT_SEGMENT 同時有 snap_date（DATE）和 snap_yyyymm（VARCHAR）；月份快照篩選和 ROW_NUMBER() 排序應使用 snap_yyyymm，不應以 snap_date 做月份等值比較（DATE 含時間成分）。M_RF_MARKET_SHARE 使用 snap_date（DATE 型）做日期篩選。
...
- 任何兩張表以個人關聯時，JOIN 條件必須同時包含 party_id 與 branch_code；只用 party_id 會導致一對多資料膨脹。
- 個人層級以 party_id + branch_code 識別；帳戶層級以 acct_nbr + prod_type_code 識別。層級由高到低：分公司 → 個人(party_id + branch_code) → 帳戶(acct_nbr + prod_type_code)。
- party_id 套用 Oracle Data Redaction：最終報表 SELECT（最外層）禁止輸出 party_id；CTE / subquery 內可保留 party_id 作為 JOIN / GROUP BY key。
- 需顯示個人識別碼時改用 party_id_mask：若來源表本身已有 party_id_mask 欄位，直接 SELECT party_id_mask；若來源表無 party_id_mask，則 JOIN DM_S_VIEW.M_AC_ACCOUNT ON <主表>.party_id = M_AC_ACCOUNT.party_id，再 SELECT M_AC_ACCOUNT.party_id_mask。
- party_id 與 party_id_mask 數值不同，不可互換：JOIN / WHERE 條件一律用 party_id；禁止用 party_id_mask 去 JOIN 其他表格的 party_id。
- ★ 代碼合法值是 table-scoped：PROD_STYPE_CODE、PROD_MTYPE_CODE、PROD_TYPE_CODE 等代碼欄位，每張表的合法值都不同，只能使用**該表** schema 定義中列出的代碼。M_AT_ACCOUNT_TXN 等跨商品彙總表的代碼清單涵蓋所有商品類別，禁止把這些代碼套用到 M_AT_STOCK_TXN、M_AT_BOND_TXN、M_AT_FUND_TXN 等個別商品明細表。
