...
- 任何兩張表以個人關聯時，JOIN 條件必須同時包含 party_id 與 brand_code；只用 party_id 會導致一對多資料膨脹。
- 個人層級以 party_id + brand_code 識別；帳戶層級以 acct_nbr + prod_type_code 識別。層級由高到低：分公司 → 個人(party_id + brand_code) → 帳戶(acct_nbr + prod_type_code)。
