...
- 資料時效：整個資料庫每日 T-1 更新，所有日期欄位（SNAP_DATE、SNAP_YYYYMM、txn_date、open_date 等）的最新可用資料為昨日（SYSDATE-1）。當使用者說「今天」、「當日」、「本日」，一律解讀為昨日（SYSDATE-1）；禁止以今日日期（SYSDATE 或 DATE 'YYYY-MM-DD' 等於今日）作為篩選上限，否則查詢結果為空。
