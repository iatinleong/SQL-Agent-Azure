...
- 日期函數：TO_DATE()、TRUNC()、ADD_MONTHS()、LAST_DAY()；字串函數：NVL()、DECODE()、SUBSTR()。日期欄位等值比較（snap_date、snap_yyyymm 等）一律使用 TRUNC(SYSDATE)-N，禁止裸用 SYSDATE-N（SYSDATE 含時間成分，會導致等值比較永遠不符）。
