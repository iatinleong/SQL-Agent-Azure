...
class ReportPlan:
    ...
    understanding: str = ""         # status="confirm" 時，LLM 對整份報表需求的完整理解摘要（業務白話 + 技術術語）
...
與使用者溝通時，用業務員聽得懂的語言描述需求；提及資料來源或篩選條件時，一律以「中文業務名稱（英文技術名稱）」格式標注，例如「帳戶主檔（M_AC_ACCOUNT）」、「有效戶（ACCT_VALID_FLAG='Y' AND CLOSE_DATE IS NULL）」，同時兼顧業務員與數科人員。
只輸出 JSON，不要其他文字。"""
